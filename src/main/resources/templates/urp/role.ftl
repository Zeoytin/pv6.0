                ...
                <div class="modal-body">
                    <form class="form-horizontal" role="form">
                        <div class="form-group">
                            <label for="role" class="col-sm-3 control-label"><span
                                        style="color: red;">*</span>角色名称</label>
                            <div class="col-sm-9">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="form-horizontal" role="form">
                        ...
                        <div class="form-group">
                            <label for="role1" class="col-sm-3 control-label"><span
                                        style="color: red;">*</span>角色名称</label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control" name="role" value="" id="role1"
                                       placeholder="请输入角色名称">
                            </div>
                        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="permissionForm()">
                        提交
                    </button>
                </div>
            </div><!-- /.modal-content -->
...
<script type="text/javascript">
    ...
    function operateFormatter(value, row, index) {
        return [
            '<button type="button" id="btn_set" class="btn btn-info">分配权限</button>  ',
            '<button type="button" id="btn_clean" class="btn btn-danger">清空角色</button>'
        ].join('');
    };
    window.operateEvents = {
        'click #btn_set': function (e, value, row, index) {
            console.log("-----分配权限-----角色id=", row.id);
            localStorage.setItem("roleId", row.id);
            findPermission();
            $('#permissionModal').modal('show');
        },
        'click #btn_clean': function (e, value, row, index) {
            console.log("-----清空权限-----角色id=", row.id);
            cleanPermission(row.id);
        }
    };
    function cleanPermission(id) {
        $.ajax({
            type: "POST",
            url: "../role/cleanPermission",
            data: {"rid": id},
            success: function (data) {
                if (data.code == 1) {
                    alert('操作成功');
                ...
                if (data.code == 1) {
                    ...
                    $.each(data, function (i, item) {
                        $("#setPermission").append(
                            "<label>" +
                            "<input name='items' type='checkbox' value='" + item.id + "'>" + item.description +
                            "</label><br /> ");
                    });
                } else {
                    alert('操作失败');
    ...
    function permissionForm() {
        ...
        var pids = arrs.join(",");
        console.log("用户uid=", roleId, "角色rids=", pids);
        $.ajax({
